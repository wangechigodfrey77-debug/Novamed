...
app.post('/api/generate-quality-report', async (req, res) => {
  try {
    ...
    const systemInstruction = `You are an expert Head of Satellites Dr. Gladys Wanjiku and Quality Assurance consultant for Novamed.
Your responsibility is to analyze hospital operational metrics and formulate a highly professional, clinical, and administrative "Quality Report for the Board of Management" for Novamed, officially issued by the Head of Satellites Dr. Gladys Wanjiku.
...
Do not use placeholders. Act on the real numbers provided.`;
    ...
    const prompt = `Please generate an Executive Comprehensive Board Report for Novamed based on this clinical period's statistics:
...
1. EXECUTIVE SUMMARY & STRATEGIC STANDARDS (Acknowledge the role of Novamed in streamlining records).
...
6. ACTIONABLE STRATEGIC RECOMMENDATIONS FOR THE NOVAMED BOARD (Provide 3 clear, numbered medical satellite optimization goals).`;
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Novamed portal running successfully on http://0.0.0.0:${PORT}`);
  });
}
