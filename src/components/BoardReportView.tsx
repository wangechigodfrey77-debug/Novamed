...
export function BoardReportView({
  ...
  const handleDownloadStructuredLedger = () => {
    ...
    exportToCSV(headers, rows, `Novamed_Operational_Report_${scale}_${new Date().toISOString().substring(0, 10)}.csv`);
  };
  ...
  const handleDownloadClinicLedger = () => {
    ...
    exportToCSV(headers, rows, `Novamed_Consultant_Report_${activeKey}.csv`);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(13);
    doc.text('NOVAMED - NOVAMED', 14, 18);
    ...
    doc.setFont('Helvetica', 'bold');
    doc.setFontSize(9.5);
    doc.text('NOVAMED EXECUTIVE TREND TIMELINES & TREASURY PORTFOLIOS', 14, 15);
    doc.setFont('Helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(71, 85, 105);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8);
      doc.setTextColor(148, 163, 184);
      doc.text(`Novamed Satellite Systems • Official Executive Audit • Page ${i} of ${pageCount}`, 14, 287);
    }

    doc.save(`Official_Novamed_Performance_Report_${scale}_${activeKey || 'current'}.pdf`);
  };
  ...
  const handleDownloadAIPDF = () => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8);
      doc.setTextColor(148, 163, 184);
      doc.text(`Novamed Satellite Systems • Official AI quality report • Page ${i} of ${pageCount}`, 14, 287);
    }

    doc.save(`Strategic_Novamed_Board_Quality_Paper_${new Date().toISOString().substring(0,10)}.pdf`);
  };
  ...
  const renderSparkline = (values: number[]) => {
    if (values.length <= 1) {
      return (
        <svg id="svg-spark-flat" className="w-20 h-8 text-slate-300" viewBox="0 0 100 30" fill="none">
    ...
    return (
      <svg id={`svg-spark-${min}-${max}`} className="w-24 h-8 text-blue-500 overflow-visible" viewBox={`0 0 ${width} ${height}`} fill="none">
        <polyline
          fill="none"
          stroke="currentColor"
          strokeWidth="2.5"
    ...
    return (
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* Chart Legend */}
          <div className="flex flex-wrap items-center gap-4 text-[10px] font-semibold text-slate-600 font-mono">
                ...
                <g key={r.periodKey}>
                  ...
                  {allReports.length < 15 || i % Math.ceil(allReports.length / 10) === 0 ? (
                    <text
                      ...
                      textAnchor="middle"
                      className="font-mono text-[9px] fill-slate-400 rotate-12 origin-top"
                    >
                      {r.periodKey}
                    </text>
                  ) : null}
  ...
  const renderMonthlyNetProfitGrowthChart = () => {
    ...
    if (data.length === 0) {
      return (
        <div id="monthly-net-profit-growth-analyser" className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
      ...
      <div id="monthly-net-profit-growth-analyser" className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-4 text-[10px] font-semibold text-slate-600 font-mono">
            ...
            <div className="flex items-center gap-1.5">
              <span className="w-3.5 h-1 bg-blue-500 rounded-sm inline-block" strokeDasharray="3 3" style={{ borderTop: '2px dashed #10b981' }} />
                ...
                <g key={d.monthKey} className="group cursor-pointer">
                  ...
                  <text
                    ...
                    className={`font-sans text-[9px] font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none ${
                      d.growthAmount >= 0 ? 'fill-blue-700' : 'fill-rose-700'
                    }`}
                  >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-1">
          {data.map((d) => (
            <div key={d.monthKey} className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-1">
              <span className="text-[10px] text-slate-400 font-bold block">{d.monthName}</span>
              <div className="flex justify-between items-center">
                ...
                <span className={`text-[9.5px] font-bold px-1 py-0.5 rounded ${
                  d.growthAmount >= 0 ? 'bg-blue-50 text-blue-700' : 'bg-rose-50 text-rose-700'
                }`}>
                  {d.growthAmount >= 0 ? '▲' : '▼'} {d.growthRate.toFixed(0)}%
                </span>
              </div>
              <div className="flex justify-between text-[9px] text-slate-400 pt-0.5 border-t border-slate-100">
...
As the Head of Satellites, Dr. Gladys Wanjiku officially submits this Quality and Operational Report for the Novamed Hospital Novamed Branch. The satellite facility continues to serve as a critical primary and specialized clinical hub. Operational records reflect positive progress in streamlining patient intake pathways and aligning clinical audits with Novamed's master database standards. The branch has successfully minimized data latency, allowing this active period's analysis to be retrieved instantly.
...
The Novamed Branch generated a robust total clinical revenue of **Ksh ${totalRevenue.toLocaleString()}** during this period. The direct contributions of the auxiliary clinical support departments are listed below:
...
## 6. ACTIONABLE STRATEGIC RECOMMENDATIONS FOR THE NOVAMED BOARD
...
2.  **Maternal Care Subsidy Campaign:** Launch a subsidized Obs/Gyn outreach clinic in the Novamed municipality during the upcoming period to expand community healthcare access.
  ...
  const renderFormattedReport = (text: string) => {
    return text.split('\n').map((line, index) => {
      ...
      if (trimmed.startsWith('###')) {
        return (
          <h4 id={`rep-custom-h3-${index}`} key={index} className="text-md font-semibold text-blue-800 mt-4 mb-2 flex items-center gap-1">
      ...
      if (trimmed.startsWith('##')) {
        return (
          <h3 id={`rep-custom-h2-${index}`} key={index} className="text-lg font-bold text-slate-800 border-b border-slate-200 pb-1 mt-6 mb-3">
            {trimmed.replace('##', '').trim()}
          </h3>
        );
      }
      if (trimmed.startsWith('#')) {
        return (
          <h2 id={`rep-custom-h1-${index}`} key={index} className="text-xl font-extrabold text-slate-950 mt-8 mb-4 border-l-4 border-blue-600 pl-3">
            {trimmed.replace('#', '').trim()}
          </h2>
        );
      }
      if (trimmed.startsWith('-') || trimmed.startsWith('*')) {
        return (
          <li id={`rep-custom-li-${index}`} key={index} className="ml-5 list-disc text-sm text-slate-700 my-1 leading-relaxed">
      ...
      if (trimmed.startsWith('|')) {
        return (
          <div id={`rep-custom-tbl-${index}`} key={index} className="font-mono text-xs bg-slate-50 border-x border-slate-100 p-2 text-slate-600 overflow-x-auto">
            {trimmed}
          </div>
        );
      }
      ...
      if (parts.length > 2) {
        return (
          <p id={`rep-custom-p-${index}`} key={index} className="text-sm text-slate-700 leading-relaxed my-2">
      ...
      return (
        <p id={`rep-custom-p-plain-${index}`} key={index} className="text-sm text-slate-700 leading-relaxed my-2">
          {trimmed}
        </p>
      );
    });
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1.5 leading-normal">
            Novamed Satellite Admin Audit Board. Select periodic metrics, download audited regulatory spreadsheets, or generate AI strategic projections.
          </p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl self-start md:self-auto border border-slate-200">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer ${
              activeSubTab === 'analytics' ? 'bg-white text-slate-950 shadow-xs' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer ${
              activeSubTab === 'ai_report' ? 'bg-white text-slate-950 shadow-xs' : 'text-slate-500 hover:text-slate-800'
            }`}
          >
        ...
        <div id="analytics-auditor-space" className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                <button
                  ...
                  className="bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-800 font-bold text-xs px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs"
                >
                ...
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-800 font-semibold text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer"
                >
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
              <span className="text-xs font-bold text-slate-500 uppercase mr-2">Filter Dept:</span>
            ...
            {periodKeys.length > 0 && (
              <div className="border-t border-slate-100 pt-4 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
                <span className="text-slate-500 font-medium">
                  Currently examining audit record: <strong className="text-slate-800 font-semibold">{activeKey || 'No recordings'}</strong>
                </span>
                
                <div className="flex items-center gap-2">
                  <span className="text-slate-400">Jump to another {scale} interval:</span>
                  <select
                    id="select-periodic-interval"
                    value={activeKey}
                    onChange={(e) => setActiveReportKey(e.target.value)}
                    className="bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-xs text-slate-700 outline-hidden font-mono focus:ring-1 focus:ring-blue-500"
                  >
                    {periodKeys.map((k) => (
          ...
          {filteredActiveReport ? (
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                {/* Card 1: Patients */}
                <div id="kpi-pt-attended" className="bg-white p-4 rounded-xl border border-slate-150 shadow-2xs hover:shadow-xs transition-all">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Patients Attended</span>
                    <Users2 className="w-4 h-4 text-blue-500" />
                  </div>
                  <span className="text-xl font-bold text-slate-900 block mt-2">{filteredActiveReport.patientsCount}</span>
                  <span className="text-[10px] text-slate-400 block mt-1">
                    Novamed registries in <strong>{activeKey}</strong>
                  </span>
                </div>

                {/* Card 2: Lab Tests */}
                <div id="kpi-tests-done" className="bg-white p-4 rounded-xl border border-slate-150 shadow-2xs hover:shadow-xs transition-all">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Lab Tests Done</span>
                ...
                {/* Card 3: Pharmacy */}
                <div id="kpi-pharmacy-served" className="bg-white p-4 rounded-xl border border-slate-150 shadow-2xs hover:shadow-xs transition-all">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Pharmacy Served</span>
                ...
                {/* Card 4: Gross Profit */}
                <div id="kpi-satellite-revenue" className="bg-white p-4 rounded-xl border border-slate-150 shadow-2xs hover:shadow-xs transition-all">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Gross Profit</span>
                    <TrendingUp className="w-4 h-4 text-blue-600" />
                  </div>
                  <span className="text-xl font-bold text-blue-700 block mt-2">Ksh {filteredActiveReport.totalRevenue.toLocaleString()}</span>
                ...
                <div id="kpi-satellite-expenses" className="bg-white p-4 rounded-xl border border-slate-150 shadow-2xs hover:shadow-xs transition-all">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">Total Expenses</span>
                ...
                <div id="kpi-satellite-netprofit" className={`p-4 rounded-xl border shadow-2xs hover:shadow-xs transition-all ${
                  filteredActiveReport.netProfit >= 0 ? 'bg-blue-50/20 border-blue-150' : 'bg-rose-50/20 border-rose-150'
                }`}>
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider font-bold">Net Profit</span>
            ...
            <div className="bg-amber-50 border border-amber-200 text-slate-700 p-4 rounded-xl text-xs flex items-center gap-2">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <button
                ...
                disabled={!activeKey}
                className="bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold text-[10px] px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 shrink-0 transition-all cursor-pointer disabled:opacity-50"
              >
            ...
            {activeKey && clinicReports.length > 0 ? (
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {clinicReports.map((c) => (
                  <div id={`clinic-card-${c.name}`} key={c.name} className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-4 hover:shadow-xs transition-all">
                    <div className="flex items-start justify-between">
                      <div>
                        <span className="text-xs font-bold text-slate-900 block">{c.name} Clinic</span>
                        <span className="text-[10px] text-slate-400">Consultant Area</span>
                      </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 flex items-end justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-xs font-bold text-blue-700">{c.patientCount} attending</span>
                      </div>
                    </div>

                    {/* Interactive trend graph sparkline 📈 */}
                    <div className="pt-2 border-t border-slate-100 space-y-1">
                      <span className="text-[9px] text-slate-400 uppercase tracking-wider block font-mono">Revenue Trend Timeline (📈)</span>
                      <div className="flex justify-center py-2 bg-white rounded-lg border border-slate-100">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">
            ...
            <div className="overflow-x-auto">
              <table id="tbl-audited-operational-timesheet" className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold font-mono tracking-wider text-[10px] uppercase">
                  ...
                  {filteredAllReports.map((r) => (
                    <tr
                      ...
                      className={`hover:bg-slate-50 transition-colors ${
                        r.periodKey === activeKey ? 'bg-blue-50/50 font-medium text-slate-900 border-l-4 border-blue-600' : ''
                      }`}
                    >
                      ...
                      <td className="p-4 text-right text-slate-500 font-mono">Ksh {r.receptionRevenue.toLocaleString()}</td>
                      ...
                      <td className="p-4 text-right font-bold text-blue-700 font-mono">Ksh {r.totalRevenue.toLocaleString()}</td>
                      ...
                      <td className="p-4 pr-6 text-center">
                        <button
                          ...
                          onClick={() => {
                            ...
                            exportToCSV(headers, rows, `Novamed_Brief_${r.periodKey}.csv`);
                          }}
                          className="bg-slate-100 hover:bg-slate-200 p-1.5 rounded-md cursor-pointer transition-all inline-flex items-center justify-center border border-slate-200"
        ...
        <div id="ai-strategic-report-space" className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Produce audited medical, revenue, and HR Quality Reports ready for the Novamed Executive Board.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-lg border border-slate-200">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-[11px] font-semibold transition-all cursor-pointer ${
                    reportScope === 'period' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-700'
                  }`}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-[11px] font-semibold transition-all cursor-pointer ${
                    reportScope === 'all-time' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-700'
                  }`}
              ...
              <button
                ...
                disabled={generating}
                className="bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs px-4 py-2.5 rounded-lg transition-all flex items-center justify-center gap-2 disabled:bg-blue-400 cursor-pointer"
              >
                {generating ? (
                  <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
              <span className="text-xs text-slate-500 block">Total Patients Attended</span>
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
              <span className="text-xs text-slate-500 block">Lab Activity & Revenue</span>
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
              <span className="text-xs text-slate-500 block">Pharmacy Dispenses</span>
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
              <span className="text-xs text-slate-500 block">Combined Satellite Income</span>
          ...
          {reportHistory.length > 0 && (
            <div id="prev-reports-container" className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[220px] overflow-y-auto p-1 border border-slate-100 rounded-lg bg-slate-50/50">
                    ...
                    >
                      ...
                      <div className="w-full z-10 pointer-events-none">
                        <div className="flex items-center justify-between w-full">
                          <span className="text-[9px] font-mono font-bold text-slate-400 uppercase tracking-widest">{rep.id}</span>
                          <div className="flex items-center gap-1.5 pointer-events-auto">
                            {isActive && (
                              <span className="bg-blue-100 text-blue-800 text-[8px] font-extrabold px-1.5 py-0.2 rounded uppercase">
                                Active View
                              </span>
                            )}
                            <button
                        ...
                        <span className="text-xs font-bold text-slate-800 block mt-1 leading-tight">
                          Report of {formattedDate}
                        </span>
                      </div>
                      <div className="w-full text-[10px] text-slate-500 flex justify-between items-center border-t border-slate-100/60 pt-1.5 mt-1.5 z-10 pointer-events-none">
                        <span>Patients: {rep.patientCount}</span>
                        <span className="font-semibold text-blue-700">Ksh {rep.totalRevenue.toLocaleString()}</span>
                      </div>
                    </div>
                  );
                })}
          ...
          {errorMsg && (
            <div id="ai-report-error" className="bg-rose-50 border border-rose-200 rounded-lg p-4 text-slate-800 flex items-start gap-2 text-xs">
          ...
          {generating && (
            <div className="bg-white border border-slate-200 rounded-xl p-12 flex flex-col items-center justify-center text-center space-y-4">
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-800">Analyzing Clinical Registers via Gemini 3.5</h3>
              ...
              <div className="w-48 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                <div className="bg-blue-600 h-full w-2/3 rounded-full animate-[pulse_1.5s_infinite]" />
              </div>
            </div>
          )}

          {!generating && reportText && (
            <div id="printed-board-paper" className="bg-white border border-slate-200 rounded-xl shadow-xs overflow-hidden">
              <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-white hover:bg-blue-700 bg-blue-600 flex items-center gap-1.5 text-xs font-bold border border-blue-700 rounded px-3 py-1.5 cursor-pointer shadow-2xs transition-all"
                  >
                  ...
                  <button
                    ...
                    className="text-slate-600 hover:text-slate-900 flex items-center gap-1 text-xs border border-slate-200 rounded px-2 py-1.5 bg-white cursor-pointer"
                  >
              ...
              <div className="p-8 md:p-12 prose max-w-none max-h-[600px] overflow-y-auto">
                <div className="text-center border-b-2 border-slate-800 pb-6 mb-6">
                  <h1 className="text-lg font-bold tracking-wide uppercase text-slate-900 font-sans">
                    Novamed Hospital Novamed Branch
                  </h1>
                  <p className="text-xs text-slate-500 uppercase tracking-widest mt-1">
                    Novamed, Nyeri County, Kenya • board of management quality assessment
                  </p>
                  {(() => {
                ...
                <div className="border-t border-slate-200 mt-12 pt-6 flex justify-between items-center text-[10px] text-slate-400 font-mono">
                  <span>Prepared Server-Side via Gemini-3.5-Flash</span>
                  <span>Novamed Novamed Systems</span>
                </div>
              </div>
            </div>
          )}
          ...
          {!generating && !reportText && !errorMsg && (
            <div className="bg-white border border-slate-100 rounded-xl p-8 text-center text-slate-400 space-y-3">
