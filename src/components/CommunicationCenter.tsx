    ...
    <div id="communication-center-container" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200">
        ...
        <form onSubmit={handleSend} className="space-y-4">
          <div>
            <label id="lbl-recipient" className="block text-xs font-medium text-slate-500 mb-1">To (Recipient)</label>
            <select
              id="select-recipient"
              value={recipient}
            ...
            >
              <optgroup label="General Broadcasts">
                <option value="all-staff">📢 All Novamed Staff (Broadcast)</option>
              </optgroup>
          ...
          <div>
            <label id="lbl-subject" className="block text-xs font-medium text-slate-500 mb-1">Subject</label>
            <input
              id="txt-subject"
              type="text"
              required
          ...
          <div>
            <label id="lbl-message-body" className="block text-xs font-medium text-slate-500 mb-1">Message Body</label>
            <textarea
              id="txt-message-body"
              required
              rows={4}
              ...
              className="w-full bg-white border border-slate-200 rounded-lg p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-hidden resize-none"
            ></textarea>
          </div>

          {success && (
            <div id="msg-send-success" className="bg-blue-50 border border-blue-100 text-blue-800 text-[11px] p-2 rounded-lg flex items-center gap-1.5 animate-pulse">
          ...
          <button
            id="btn-send-message"
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs py-2 rounded-lg transition-all flex items-center justify-center gap-2"
          >
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-lg p-3.5 mt-4 text-[10px] text-slate-400 flex items-start gap-2 leading-relaxed">
            ...
            myMessages.map((msg) => {
              ...
              return (
                <div
                  id={`msg-card-${msg.id}`}
                  key={msg.id}
                  className={`border p-4 rounded-xl transition-all bg-white hover:border-slate-300 ${
                    isBroadcast
                      ? 'border-amber-200 shadow-xs'
                      : isFromMe
                      ? 'border-blue-100 border-l-4 border-l-blue-600'
                      : 'border-slate-200 border-l-4 border-l-slate-600'
                  }`}
                >
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {!isBroadcast && isFromMe && (
                          <span className="bg-blue-50 text-blue-800 text-[9px] px-1.5 rounded-sm">
                            Sent
                          </span>
                        )}
                      </div>
                  ...
                  <p className="text-xs text-slate-600 mt-2.5 whitespace-pre-line leading-relaxed bg-slate-50/50 p-2.5 rounded border border-slate-100/50">
                    {msg.content}
                  </p>
                </div>
              );
