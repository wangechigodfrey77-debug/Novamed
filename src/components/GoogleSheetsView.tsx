...
export function GoogleSheetsView({
    ...
    try {
      ...
      if (!activeSpreadsheetId) {
        ...
        const createResponse = await fetch('https://sheets.googleapis.com/v4/spreadsheets', {
          ...
          body: JSON.stringify({
            properties: {
              title: "Novamed Hospital - Comprehensive Management Ledger"
            },
            sheets: [
      ...
      if (syncOverview) {
        ...
        updateData.push({
          ...
          values: [
            ["Novamed Hospital - Executive Overview", "", "", ""],
  ...
  return (
    <div id="sheets-integration-workspace" className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-sans">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs h-fit space-y-5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500">Secure Workspace Connector</p>
          </div>
        </div>
        ...
        {!accessToken ? (
          <div className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-[11px] leading-relaxed text-slate-600">
              <span className="flex items-center gap-1.5 font-bold text-slate-800 mb-1">
                <Lock className="w-3.5 h-3.5 text-slate-500" /> Authorized Secure Connection
              </span>
            ...
            <button
              ...
              className="w-full gsi-material-button text-xs py-1 hover:brightness-95 active:brightness-90 transition shadow-xs focus:ring-1 focus:ring-blue-500 cursor-pointer flex items-center justify-center border border-slate-200 rounded-lg bg-white text-slate-800 font-semibold"
            >
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-150 rounded-lg p-3 text-[11px] leading-relaxed text-blue-800">
              <span className="flex items-center gap-1.5 font-bold text-blue-950 mb-0.5">
                <ShieldCheck className="w-4 h-4 text-blue-600" /> Active Session Linked
              </span>
              Your Google identity is verified and connected to the sheets connector with authorized permission.
              <div className="text-[10px] text-blue-700/85 mt-2 break-all font-mono">
                Email: {userEmail}
              </div>
            </div>
            ...
            <button
              ...
              className="w-full border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold py-2 rounded-lg text-xs transition cursor-pointer text-center"
            >
        ...
        <form onSubmit={handleSaveSheetId} className="space-y-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-lg py-1.5 pl-2.5 pr-8 text-xs focus:ring-1 focus:ring-blue-500 outline-hidden font-mono"
            />
            {spreadSheetId && (
              <button
                ...
                className="absolute right-2.5 top-1.5 text-slate-400 hover:text-slate-600 cursor-pointer font-bold"
              >
                ×
              </button>
            )}
          ...
          <button
            ...
            type="submit"
            className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 text-[10px] font-bold py-1.5 rounded-lg transition-colors cursor-pointer text-center"
          >
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs lg:col-span-2 space-y-6">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 leading-relaxed mt-0.5">
            Choose exactly which sections of Novamed hospital records database are translated to the target sheets. Selecting unchecked sheets will omit updates.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs leading-relaxed">
          ...
          <label 
            className={`border rounded-lg p-3 flex items-start gap-3 select-none transition-all cursor-pointer ${
              syncOverview ? 'bg-blue-50/40 border-blue-200 text-slate-900' : 'bg-slate-50/50 border-slate-150 text-slate-500'
            }`}
          >
            <input 
            ...
            <div>
              <div className="font-bold flex items-center gap-1">
                <BarChart3 className="w-3.5 h-3.5 text-blue-600" /> 1. Executive KPIs & Overview
              </div>
              <div className="text-[10px] text-slate-500 mt-0.5">Consolidates cash flows, patient counts, inventory SKUs, and operation health summaries.</div>
            </div>
          </label>

          <label 
            className={`border rounded-lg p-3 flex items-start gap-3 select-none transition-all cursor-pointer ${
              syncPatients ? 'bg-blue-50/40 border-blue-200 text-slate-900' : 'bg-slate-50/50 border-slate-150 text-slate-500'
            }`}
          >
            <input 
            ...
            <div>
              <div className="font-bold flex items-center gap-1">
                <Users className="w-3.5 h-3.5 text-blue-600" /> 2. Clinical Patient Cohorts
              </div>
              <div className="text-[10px] text-slate-500 mt-0.5">Synchronizes active medical registrations ({patients.length} records), consultation types, and contact details.</div>
            </div>
          </label>

          <label 
            className={`border rounded-lg p-3 flex items-start gap-3 select-none transition-all cursor-pointer ${
              syncStock ? 'bg-blue-50/40 border-blue-200 text-slate-900' : 'bg-slate-50/50 border-slate-150 text-slate-500'
            }`}
          >
            <input 
              id="chk-sync-stock"
            ...
            <div>
              <div className="font-bold flex items-center gap-1">
                <Pill className="w-3.5 h-3.5 text-blue-600" /> 3. Pharmacy Stock Catalogs
              </div>
              <div className="text-[10px] text-slate-500 mt-0.5">Registers stock counts ({stock.length} lines), current unit prices, and alert warning thresholds.</div>
            </div>
          </label>

          <label 
            className={`border rounded-lg p-3 flex items-start gap-3 select-none transition-all cursor-pointer ${
              syncLab ? 'bg-blue-50/40 border-blue-200 text-slate-900' : 'bg-slate-50/50 border-slate-150 text-slate-500'
            }`}
          >
            <input 
              id="chk-sync-lab"
            ...
            <div>
              <div className="font-bold flex items-center gap-1">
                <Beaker className="w-3.5 h-3.5 text-blue-600" /> 4. Laboratory Investigation Queue
              </div>
              <div className="text-[10px] text-slate-500 mt-0.5">Streams diagnostic testing log records ({labTests.length} queues), status states, and fees.</div>
            </div>
          </label>

          <label 
            className={`border rounded-lg p-3 flex items-start gap-3 select-none transition-all cursor-pointer ${
              syncExpenses ? 'bg-blue-50/40 border-blue-200 text-slate-900' : 'bg-slate-50/50 border-slate-150 text-slate-500'
            }`}
          >
            <input 
            ...
            <div>
              <div className="font-bold flex items-center gap-1">
                <Receipt className="w-3.5 h-3.5 text-blue-600" /> 5. Operations Expenses Ledger
              </div>
              <div className="text-[10px] text-slate-500 mt-0.5">Documents hospital expenses ({expenses.length} postings), descriptive billing, and transaction dates.</div>
            </div>
          </label>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-5 border border-slate-200 space-y-4">
          ...
          {accessToken ? (
            <div>
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold text-xs py-3 rounded-lg shadow-sm font-sans flex items-center justify-center gap-2 transition cursor-pointer"
              >
            ...
            <div className="text-center py-2">
              <p className="text-[11px] text-slate-500 mb-1 font-medium">Sheets integration controls are inactive</p>
              <p className="text-[10px] text-slate-400">Please authorize your Google Account in the sidebar panel to unlock live streaming updates.</p>
            </div>
          )}
          ...
          {syncStatus === 'syncing' && (
            <div className="space-y-2">
              <span className="text-[10px] font-bold text-blue-800 tracking-wider flex items-center gap-1.5">
          ...
          {syncStatus === 'success' && syncedSheetUrl && (
            <motion.div 
              ...
              className="bg-blue-50 border border-blue-150 rounded-lg p-4 space-y-3"
            >
              <div className="flex items-start gap-2.5">
                <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
                <div className="text-xs">
                  <span className="font-bold text-blue-950 block">Google Sheets Sync Completed Successfully!</span>
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-1">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="bg-blue-600 text-white hover:bg-blue-700 font-bold text-xs py-2 px-4 rounded-lg flex items-center justify-center gap-1.5 transition-all text-center"
                >
                ...
                <button
                  ...
                  className="bg-white border border-blue-250 hover:bg-blue-50/50 text-blue-800 font-bold text-xs py-2 px-4 rounded-lg flex items-center justify-center gap-1 transition-all"
                >
                  Copy Sheet Link
                </button>
              </div>
